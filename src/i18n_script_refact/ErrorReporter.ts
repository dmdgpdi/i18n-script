...
export class ErrorReporter implements IErrorReporter {
  ...
  private generateCodeSnippet(filePath: string, line: number, column: number): string {
    try {
      const content = this.getFileLines(filePath);
      return codeFrameColumns(content, {
        start: { line, column }
      }, {
        highlightCode: true,
        linesAbove: 2,
  ...
  private formatError(error: CodeError, filePath: string): CodeError {
    const codeSnippet = this.generateCodeSnippet(filePath, error.line, error.column);
